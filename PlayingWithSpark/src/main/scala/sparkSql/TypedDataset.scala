...
object TypedDataset extends App with Context {
  ...
  val response = dataFrameReader.option("header", "true")
    .option("inferSchema", value = true)
    .csv("in/2016-stack-overflow-survey-responses.csv")

  val responseWithDF = response.select("country", "age_midpoint", "occupation", "salary_midpoint")

  import sparkSession.implicits._

  val typedDataset = responseWithDF.as[Response]
